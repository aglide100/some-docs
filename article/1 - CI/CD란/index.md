...
지금까지 자료를 조사해오면서 공통적으로 느꼈던 부분은 한가지가 있습니다.

바로 CI/CD가 단순히 코드를 배포하는 것이 아닌 개발론에 더 가깝다는 것입니다.

어찌보면 단어 자체가 개발론에서 출발하였으니 당연한 수순일지도 모릅니다.

그렇기에 완벽한 정답은 없지만 최선이라고 생각되는 부분은 참여율인데 이를 위해서는 해당 요소에 대한 `접근성`이 가장 중요한 요소라고 생각합니다.

접근성이 높다는 것은 많은 사람이 알아차리고 볼 수 있으며 자연스럽게 관심 또한 가질 수 있기 떄문입니다.

그렇게 높아진 관심은 집단적 코드 소유권이라는 개념을 더욱 주입이 가능하며 이러한 규칙이 더 잘 굴러갈 수 있다고 생각됩니다. 

# 벤더 락인된 CI/CD 툴

개인적으로는 어떤 것이 가장 좋은 CI툴이라고 할 수는 없지만 접근성이 좋은 것이 최선이라고 생각됩니다.

가령 Gihub/Gitlab와 잘 통합되어 움직이는 `Github Action`이나 `Gitlab Runner`가 좋은 선택이라고 생각됩니다.

해당 툴들은 따로 봇과 같이 추가를 하지 않더라도 Web UI에서 진행 사항을 확인할 수 있으며, 이는 PR에서 `Mergeable`을 수행할 수 있는 파이프라인을 확인 가능하기 때문입니다.


아래는 Naver에서 소개하는 GitHub Actions를 이용한 코드 리뷰 문화 개선에 대한 글에서 발췌한 내용 중 일부인데
<img src="./img/05.png">

해당 게시글에서는 Reminder를 도입하여 PR에 리뷰가 얼마나 잘 되는지를 확인한 글이며, Github Action을 통해 해당 기능을 구현하였다고 합니다.

이는 Tag와 인디케이터를 통해 최소한의 테스트를 거치었는지, 얼마나 거치었는지를 시각적으로 바로 확인할 수 있었고 이러한 부분은 높은 PR과 코드 리뷰 참여율로 이어졌다고 합니다.

이렇게 높은 접근성은 앞에서 서술하였던 CI 극장과 같은 현상을 피할 수도 있으며 어찌보면 집단적 코드 소유권의 이행이라고 볼 수 있다고 생각합니다.


이러한 접근 방식은 앞서 얘기했던 `Trunk-based`나 `github-flow`와 같은 개발론과 같이 사용하기도 좋다고 생각됩니다.
